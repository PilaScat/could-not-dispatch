...
def resolve_interpreter() -> str:
    ...
    raise RuntimeError("No Python interpreter could be located to start the fallback stream.")
...
def find_servers(port: int) -> list[int]:
    wanted = str(int(port))
    found: list[int] = []
    try:
    ...
    for entry in entries:
        ...
        pid = int(entry)
        parts = read_cmdline_parts(pid)
        if SERVER_MODULE not in parts or "--port" not in parts:
            continue
        index = parts.index("--port")
        if index + 1 < len(parts) and parts[index + 1] == wanted:
            found.append(pid)
    return found


def terminate_strays(port: int, keep_pid: object = None) -> int:
    spared = coerce_pid(keep_pid)
    stopped = 0
    for pid in find_servers(port):
        if pid == spared:
            continue
        if terminate(pid):
            stopped += 1
...
def is_running(pid: object, expected_token: str | None = None) -> bool:
    ...
    if expected_token and not matches_token(target, expected_token):
        return False
    try:
        finished, _ = os.waitpid(target, os.WNOHANG)
        if finished == target:
            return False
